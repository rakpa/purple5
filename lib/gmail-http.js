...
export async function loadConnection(authorization, userId) {
  ...
  if (!result.ok) {
    ...
    if (/does not exist|schema cache|Could not find/i.test(detail)) {
      const error = new Error(
        "Gmail table is missing. Run supabase-gmail-setup.sql in the Supabase SQL editor."
      );
      error.status = 500;
      throw error;
    }
  ...
  if (!result.ok) {
    ...
    const error = new Error(
      /does not exist|schema cache|Could not find/i.test(detail)
        ? "Gmail table is missing. Run supabase-gmail-setup.sql in the Supabase SQL editor."
        : result.data?.message || "Could not save Gmail connection."
    );
    error.status = 500;
    throw error;
  }
...
async function gmailGet(accessToken, path) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    const error = new Error(data.error?.message || `Gmail API failed (${response.status})`);
    error.status = response.status === 403 ? 403 : 500;
    throw error;
  }
  return data;
}
...
export async function listStatementMessages(accessToken, query) {
  const q = encodeURIComponent(
    query || 'subject:"Wyciąg elektroniczny" filename:pdf has:attachment'
  );
  const listed = await gmailGet(accessToken, `messages?q=${q}&maxResults=8`);
