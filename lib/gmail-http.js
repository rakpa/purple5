...
export async function googleAccessScopes(accessToken) {
...
export function hasGmailReadonlyScope(scopes) {
  return /gmail\.readonly|\/gmail\./i.test(String(scopes || ""));
}

export function gmailApiError(data, status) {
  const apiMessage = data?.error?.message || `Gmail API failed (${status})`;
  const reason = [
    data?.error?.status,
    ...
    .filter(Boolean)
    .join(" ");
  const hay = `${apiMessage} ${reason}`;
  const enableUrl = apiMessage.match(/https:\/\/console\.[^\s]+/)?.[0];
  let code = "gmail_error";
  let message = apiMessage;

  if (status === 401) {
    code = "gmail_token_expired";
    message = "Google sign-in expired. Click Allow Gmail access, then fetch again.";
  } else if (/ACCESS_TOKEN_SCOPE_INSUFFICIENT|insufficient authentication scopes|insufficientPermissions/i.test(hay)) {
    code = "gmail_scope_missing";
    message =
      "Google has not granted Gmail access yet. Click Allow Gmail access, choose your account, and allow reading statement emails.";
  } else if (enableUrl || /accessNotConfigured|has not been used in project|API has not been used|is disabled/i.test(hay)) {
    code = "gmail_api_disabled";
    message = enableUrl
      ? `Gmail API is off for this Google Cloud project. Open this link, click Enable, wait about a minute, then fetch again: ${enableUrl}`
      : "Gmail API is disabled. Open https://console.cloud.google.com/apis/library/gmail.googleapis.com — use the same Google Cloud project as Sign in with Google — click Enable, wait about a minute, then fetch again.";
  }

  const error = new Error(message);
  error.status = status === 401 || status === 403 ? status : 500;
  error.code = code;
  return error;
}
...
async function gmailGet(accessToken, path) {
  ...
  const data = await response.json().catch(() => ({}));
  if (!response.ok) {
    throw gmailApiError(data, response.status);
  }
  return data;
}
