...
export async function handleGmailRequest(req, res) {
  try {
    ...
    if ((action === "search" || action === "statements") && (req.method === "POST" || req.method === "GET")) {
      const userId = requireUserId(authorization);
      const body = req.method === "POST" ? await readBody(req) : {};
      const connected = await loadConnection(authorization, userId).catch(() => null);
      const sessionToken =
      ...
      if (!accessToken) {
        send(res, 400, {
          error:
            "Google has not granted Gmail access yet. Click Allow Gmail access, then fetch statements.",
          code: "gmail_scope_missing",
        });
        return;
      }
      const scopes = await googleAccessScopes(accessToken).catch(() => "");
      if (scopes && !hasGmailReadonlyScope(scopes)) {
        send(res, 403, {
          error:
      ...
      if (action === "search") {
        ...
        for (const message of messages) {
          statements.push(await getMessageSummary(accessToken, message.id));
        }
        send(res, 200, {
          ...
          message: statements.length
            ? undefined
            : "No matching statement emails. Search used: " + query,
        });
        return;
      }
      ...
      if (!password) {
        send(res, 400, {
          error: "Enter the statement PDF password, then open the selected email.",
        });
        return;
      }
      const selectedId = typeof body.message_id === "string" ? body.message_id : "";
      const messages = selectedId
        ? [{ id: selectedId }]
        : await listStatementMessages(accessToken, query);
      ...
      for (const message of messages.slice(0, 8)) {
        ...
        for (const file of downloaded.attachments) {
          ...
          statements.push({
            ...
            bank: parsed.bank || inferBank(downloaded.from, downloaded.subject),
    ...
    send(res, error.status || 500, {
      error: error.message || "Gmail request failed",
      code: error.code,
    });
  }
}
